...
1. Re-ground: `cli impls --chunk <c>` for each chunk; `cli leaderboard`; read the current best from `archive/genomes.jsonl` + the `evolve-insights` memory.
2. Brief: regenerate Codex briefs (`python -m evolve.inventors <chunk>`), add an addendum with the incumbent + last round's deltas.
