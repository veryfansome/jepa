...
- **`ps` absent from 6/12 v2 images** — debian:stable-slim, python:3.12-slim, node:22-slim, fedora, rockylinux, httpd — including BOTH final-test images and inner-val fedora (mariadb has procps). Consequence under §9.3 coverage rules as-designed: ps-cells cannot reach final-test coverage and land report-only there; the automaton signal survives on the 6 ps-capable train images + mariadb (inner). **OPEN DECISION UD-9 (awaiting sign-off)**: vendored static-busybox `ps` installed by the unrecorded bootstrap (the `after`-helper precedent) would (a) restore final-test ps coverage, (b) give ONE canonical ps format on all images (largely dissolving the DG-5 format-variance surface), (c) leave determinism untouched (sha-pinned host artifact, version identity); cost: `ps` becomes an installed artifact rather than stock tooling. Alternatives: accept report-only-on-final as audited; defer install-as-observable-action to v4 (recorded in BACKLOG).
