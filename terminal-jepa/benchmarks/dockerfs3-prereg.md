...
| DG-5 | process-arm entropy: ps distinct renders; single-render mass; job-state mixture | ≥25/image; ≤25%; {run,stop,term,mixed} ≥15% each — **job-state mixture floor WAIVED/DEFERRED to v3.1 (§4.8/S5, 2026-07-23)**: the automaton is starved at mint scale (≈1 kill / 360 seqs; after/kill get ≈0 draws), so `{run,stop,term,mixed}≥15%` is unsatisfiable; the process arm stays present as fitness-excluded/instrument | pilot P2/P3 | time |
...
The SST reads OBS_CAP=1600-char renders; the encoder's 256-token window covers ~1000 chars — the asymmetry is conservative (it strengthens the baseline) and is DISCLOSED here rather than claimed away. Companion floors are gated (G-⊥: ≥40% SST-undetermined eval steps per content verb AND ≥15% determined per content verb — one denominator, mint-scale assert; joint satisfiability with ≥500 steps/verb arithmetic-checked at pilot). [§4.8 AMENDMENT 2026-07-23 (S1): the ≥40%-undetermined floor is SATISFIED-BY-CONSTRUCTION with margin (max SST-determined content cell `grep|hit|native` = 0.4626 → 54% undetermined; the 7-arm-max is the operative enforcement) and is recorded as such rather than a live gate; the ≥15%-determined COMPANION is DROPPED as a hard content-cell gate (its literal per-verb form fails-closed and guts content mass — a NO-GO tripwire) and reframed as a report-battery diagnostic. See §4.8/S1.] Freeze-then-audit: SST rules + template tables are authored from train-pilot renders + docs only, content-hashed, frozen pre-mint; post-freeze edits = dated amendment + re-baseline. Calibration-on-v2 (not v1) is a dated deviation note (§3.1).
...
2. **Per-step echo purge in content cells (Finding 1, a v2 command-echo-class recurrence).** Cell-MEAN aggregation lets a per-step minority of command-echo readbacks into content cells; the leak is concentrated where an `echo>`-created workspace file is later `mv`/`rm`-touched, flipping `state_scope` created→mutated and BYPASSING the ws_observed split → the echo readback lands in `cat|hit|mutated` content. Pooled command-uncovered leak = **8/2244 = 0.36%** (margin impact ≲0.004). FIX at freeze: the harness's classes.json→pseudo-verb rewrite (§9.5) additionally masks any content-cell STEP whose axis-2′ (recomputed at eval, incl. through-mutation echo propagation) ≥ the frozen threshold to an excluded `<cell>-echo` pseudo-verb (generalizing grep-miss) — a per-step purge, not per-cell-mean. Zero command-echo in content, by construction. IMPLEMENTED (`harness._v3_cell_verbs`, reusing `class_measure.axis2p_seq`/`_canon_output` verbatim; eval-time a2p matches the measurement bit-exactly incl. oou-skip parity — review F2 fixed; the arm loader now checks per-seq (image,len) identity — review F1 fixed). **VERIFY-AT-MINT (review F3):** the a2p≥0.96 purge fires on ALL history-derivable readbacks, not only command-echo — it also removes within-trajectory revisit re-reads (obs-derivable), so the purged footprint (~9% of content steps on the pilot) far exceeds the 0.36% command-only leak estimate. Symmetric (WM and every baseline see the same purged pool → no direct margin bias), but content mass shrinks: the freeze must re-check **G-COV (≥500 content steps/verb/split, post-purge)** and **DG-9** at the mint's 600 seqs/image — the 0.36% figure does NOT bound the purge footprint.
...
### 4.7 CLASS-FREEZE AMENDMENT (dated 2026-07-23) — `dockerfs3-classes.json` FROZEN (freeze-order step 7)

The class table is FROZEN by this dated amendment on the class-freeze pilot (60 seqs/image × 6 images = 360 seqs / 10,083 steps: 4 train — alpine busybox-ash, debian:stable-slim dash, node:22-slim dash, redis:7-alpine busybox-ash — + 1 inner-val fedora bash + 1 final-test rockylinux bash; arm=full, seed 0, `--pin-digests`, `out_of_universe=0`/`bad_state_scope=0`), via `benchmarks/class_measure.py` (v3-4axis-1) @ `axis2p_thresh=0.96`, encoder `enc_e5_base`. It resolves the four §4.6 calibrations: two resolve cleanly (axis-2′=0.96; the per-step echo purge), two become MINT-CONFIRMED gates (θ1 re-pin; composed-content DG-9). The 12-image mint STAMPS this file's sha into `summary.json` and re-confirms G-COV/DG-9 post-purge at 600/900 seqs/image; it does NOT re-derive the table. `benchmarks/class_measure.py` re-run over the collect-root reproduces the frozen table's tool fields + all 71 rows + class assignments + `class_counts` BIT-FOR-BIT (deterministic, seed-free; foil sampling pinned seed 0); the `status`/`frozen_at`/`_freeze`/`_freeze_measure` wrapper keys are freeze metadata `bench_versions.resolve()` ignores (it reads only `rows`).

**Frozen file + sha.** `benchmarks/dockerfs3-classes.json`, `status: FROZEN`, `frozen_at: 2026-07-23`, `bench_version: dockerfs3-v3.0`. **classes_sha (sha256 of the file bytes) = `08b31deeb16269c2a9d2df338c35d6a6a2f6e733c36d34c7ee5e1c853a2c24e4`** — the number `bench_versions.resolve()` asserts a v3 root's `summary.json.classes_sha` matches verbatim (fail-closed, §1.1); the mint stamps the same sha. Class counts: **content 15 · semi-echo 7 · ack 16 · sim 4 · echo/const 1 · under-floor 28** (71 cells; 43 measured ≥30-pair, 28 fail-closed under-floor).

**Frozen thresholds.** axis-2′ (`axis2p_thresh`) = **0.96** — recalibrated from the 0.656 prior (§2 item 3 / §4.6 calibration 1): returns linked-grep hits to content (`grep|hit|native` a2′=0.839, `grep|hit|mutated` a2′=0.827) while genuine workspace echo-loop readbacks stay excluded (`cat|hit|created-obs`, `readlink|hit|*` a2′=1.000 → semi-echo). axis-2 (`axis2_thresh`) = 0.656 (unchanged). axis-1 θ1 (`axis1_theta1`) = **0.59 FINAL** — the §4.6 "θ1 ≥ ~0.46" prior is REFUTED at P3 scale (`cat|hit|native` a1=0.473, `tail|hit|native` 0.488, `stat|hit|native` 0.473, `head|hit|native` 0.506 all sit above 0.46; a 0.46 floor would eject them and gut content mass); the v2 midpoint 0.59 is retained (nearest content cell head 0.506 → 0.084 headroom; `uname|hit|native` 0.811 correctly echo/const). axis-3 (`axis3_sim`) = **0.90** (unchanged). Coverage floor = **30** cross-image pairs/cell + the **≥500** post-purge content-steps/content-verb/split G-COV floor. ack 0.95 / ≤5 chars; axis-4 content 0.80 / report 0.50 (no twin → ceiling ≡ 1.0). The per-step echo purge (§4.6 Finding-1) is IMPLEMENTED in `harness._v3_cell_verbs` at this same 0.96 and reuses `class_measure.axis2p_seq`/`_canon_output` verbatim, so the eval-time content pool is consistent-or-stricter than this table.

**Frozen content-cell set (15 cells; ONLY these enter fitness — the harness pools content CELLS, not verbs).** SOLID (8) — post-purge G-COV ≥500 in every split at mint scale (×20): `ls|hit|native`, `cat|hit|native`, `ls|hit|mutated`, `head|hit|native`, `stat|hit|native`, `cat|hit|mutated`, `tail|hit|native`, `find|hit|native`. PROVISIONAL-COVERAGE (7) — content at pilot (≥30-pair floor met pre-purge, all axes content-shaped at 0.96) but post-purge G-COV MARGINAL, confirmed at the mint 600/900 seqs/image: `grep|hit|native`, `grep|hit|mutated`, `find|hit|mutated`, `cat|hit|created`, `pipe:ls|head|ok|native`, `pipe:ls|tail|ok|native`, `pipe:cat|tail|ok|native`. Content verbs: cat, find, grep, head, ls, stat, tail + 3 composed pipe families. Frozen-excluded (clearly out, per the §9.2 ordered precedence): semi-echo (`cat|hit|created-obs`, `cd|ok|native`, `echo|hit|native`, `readlink|hit|created`/`created-obs`, …), sim (`cat|miss|mutated` a3=0.945 deterministic dialect error surface / 421 steps, `pwd`/`ps` a3=1.0), echo/const (`uname|hit|native` a1=0.811), ack (all redir/mkdir/touch/rm/mv/ln `|ok`, the `|miss` acks); 28 under-floor fail-closed.

**MINT-CONFIRMED gates (do NOT block the freeze; asserted at the mint — §11 GO criterion 4).**
- **G-COV** (≥500 post-purge content steps/verb/split): at pilot ×20 the 6 simple verbs (ls, cat, head, stat, tail, find) PASS all splits; **grep** clears TRAIN only (train 520, inner 180, final 280 @600) → grep held-out content falls to the §9.3 G-COV cross-image battery or is reported under-floor on inner/final; the **3 composed pipe families** MISS all splits (≤440 @600) → §9.3 battery, not the per-verb floor. Purge footprint disclosed (review-F3): 909 steps = **16.2% of pre-purge content / 9.0% of all steps** — the a2′≥0.96 purge removes obs-derivable within-trajectory revisit re-reads too, not only the 0.36% command-echo estimate; symmetric across WM + every baseline (no direct margin bias).
- **DG-9 composed** (composed content ≥6% of post-purge content steps): **2.05% measured post-purge** (96/4693 = pipe:ls|head 35 + pipe:ls|tail 33 + pipe:cat|tail 28) — MEASURED (not the 0% draft artifact) but below floor; policy-weight-driven and ~scale-invariant (projects ≈3% at mint). STANDING BREACH RISK; re-measured at the full mint. **If it still breaches, the §4.3 pre-registered fallback fork applies** (the separately-embedded delta channel, a v3.0-BLOCKING redesign) OR the G3 composition weights are lifted before GO; the mint stays blocked until DG-9 passes or the fork is ratified by dated amendment. **DG-9 mut-affected** content = **13.2% post-purge** (621/4693; per-image 12.0–15.2%) PASS (≥10%) — resolves at pilot.
- **θ1 scale-sensitivity**: axis-1 is scale-sensitive (top-1 rose cat 0.450→0.473, tail 0.424→0.488 from the 40-seq draft to this 60-seq pilot); the θ1 ejection-safety invariant (NO frozen content cell's mint axis-1 top-1 ≥ 0.59) is re-asserted at the 12-image mint; a content verb crossing 0.59 at mint is an echo/const finding requiring a dated re-freeze amendment.

**Also frozen at this step (sha-pinned; post-freeze edit = dated amendment + re-baseline — §6 step 7 / §11 criterion 3).** `realenv/render_canon.py` (version identity, the pre-perception render mask) sha256 = `385115f503358b3372508aff50566489d19d78f58f2dd197ccb9db7acd4d3fcd`; `realenv/shell_state.py` (the SST — one tracker, two modes) sha256 = `7909c9e54b416c36486255cfdec322e83ee915ae2a131e1d8f54c2e73a11589f`; `realenv/verbsig.py` (the ONE sig/mode/cell labeler) sha256 = `fc4fed0f39890fd29af2aa23faaa7a39bfedc343dc8798207a0135dd90ce3dea`; `benchmarks/class_measure.py` (the frozen four-axis measurement tool) sha256 = `847ba8381e9eef956fc7af4f76f1b720a26d560ac97aa0a25458e7ff0ca1ff4e`.

**class_measure ↔ harness ws_observed (bookkeeping divergence, NO leak).** The collector leaves `meta.ws_observed` absent on readback steps; class_measure recomputes it from the SST (splits `created`→`created-obs`), the harness reads meta (absent → blind `created`). Consistent BY SUBSUMPTION: every `created-obs` step has a2′=1.0 ≥ 0.96, so the harness per-step purge excludes exactly the steps class_measure calls semi-echo (harness content pool consistent-or-stricter than this table; cat-created 22 ≤ 30). The `…|created-obs` rows are informational — the harness never emits that cell key. Post-purge command-echo in content = 0 (all 40 `cat|hit|mutated` mv/rm-relabel command-echoes purged).

### 4.8 GO-GATE PRE-MINT AMENDMENTS (dated 2026-07-23) — B1/S2 code, S1/S4/S5 dispositions

The GO-gate convergence adjudication returned GO-WITH-FIXES (1 blocker + 5 serious, all bounded, no world-model redesign / no re-collection). This amendment records the dispositions; the round-trip scorability test on a tiny minted root is the pre-mint precondition (§11 criterion 7).

- **B1 (code, landed).** The mint now stamps `summary["classes_sha"]` (fail-closed asserted == the pinned `08b31deeb16269c2a9d2df338c35d6a6a2f6e733c36d34c7ee5e1c853a2c24e4`) and `summary["policy_sha"]` (the `lexicon_hashes()` content hash, §1) in `collect_docker.collect()`'s v3 block; `reencode`/`mv_encode` copy both forward into `cache_meta.json`. `bench_versions.require_v3_cache` now rejects FALSY stamps (not only mismatched), closing the `None==None` vacuous pass. A drifted `dockerfs3-classes.json` aborts the mint before any trajectory is written. (`realenv/collect_docker.py::_v3_classes_sha`/`_v3_policy_sha`; `evolve/bench_versions.py::require_v3_cache`; tests in `tests/test_collect_v3.py` + `tests/test_v3_scoring_gates.py`.)

- **S2 (code, landed) — executable per-split G-COV coverage-demotion.** The MINT-CONFIRMED G-COV floor (≥500 post-purge content steps/verb/split) is now an EXECUTABLE scoring-time gate, not report-only prose: `harness._v3_cell_verbs` counts each content cell's surviving steps IN THE CURRENT eval split and rewrites every step of any content cell below the 500-step floor to the excluded `<cell>-lowcov` pseudo-verb (generalizing the `-echo`/`-miss` rewrites), routing it out of the pooled fitness margin into the §9.3 report battery. Per-split by construction (a cell may be content in inner but low-cov in final). The frozen `dockerfs3-classes.json` (`content_cell_set`, G3 weights) is UNTOUCHED — this is a scoring-layer demotion, not a re-freeze/re-pilot. At mint scale it demotes the held-out `grep|hit` cells (grep clears ≥500 only in train) and any of the 3 composed pipe families still under floor. Demoted cells are surfaced in the score result's `v3_diag.lowcov` diagnostic. (`evolve/harness.py::_demote_lowcov` + `_V3_GCOV_FLOOR=500`.) This makes the §4.7 "grep held-out content falls to the G-COV battery / composed pipes → §9.3 battery" prose an enforced code path.

- **S1 (doc) — G-⊥ satisfied-by-construction; the ≥15%-determined companion reframed, NOT hard-implemented.** The G-⊥ ≥40% SST-undetermined floor (§4.5) is SATISFIED-BY-CONSTRUCTION with margin: the operative 7-arm-max enforcement means a content cell is bankable only if some arm determines it, and the max SST-determined content cell is `grep|hit|native` at determined-fraction **0.4626 → 54% undetermined** (≥40% floor met with 14pt margin; every other content cell is more undetermined). G-⊥ is therefore recorded as satisfied-by-construction rather than a live fail-closed gate — no separate G-⊥ gate code ships. The ≥15%-determined COMPANION is DROPPED as a hard content-cell gate: its literal per-verb form fails-closed and guts content mass (5/6 content verbs sit below a 15%-determined bar under the conservative OBS_CAP asymmetry, §4.5), and hard-implementing it is a pre-registered NO-GO tripwire. It is reframed as a report-battery diagnostic: the per-cell SST-determined fraction is recorded (already available from the sst arm's `determined` mask) but never gates fitness. §4.5's parenthetical "AND ≥15% determined per content verb" is superseded by this disposition.

- **S4 (doc) — §16 render_canon interposition + `z_delta`/`exit_cls`/meta-emission DEFERRED to v3.1; §4.2 "config change" corrected.** The §7 checklist items for `reencode.py`/`mv_encode.py` (render_canon re-fold interposition, `exit_cls` computation, delta-text recompute+encode into `z_delta`, `pre_obs_step`) are DEFERRED to v3.1. This is fitness-benign in v3.0: `render_canon.canon` is a documented NO-OP on the collector's store-time-masked bytes (the §5.5 amendment moved every `-l` mtime/pseudo-fs mask to STORE-time), so real `z_obs` and the canon'd SST/wtm arm renders already share one embedding space (G-EMB holds); the cell-defining meta columns are RECOMPUTED at score-time in `_v3_cell_verbs` (the cache only accelerates); and `z_delta`/`exit_cls` are TARGET-ONLY, dormant, with no sanctioned consumer (the strip seam keeps them structurally invisible to genome code). CORRECTION to §4.2: the claim that the v3.1 aux-channel flip "ships so the v3.1 flip is a config change" is WRONG — emitting `z_delta`/`exit_cls` and interposing render_canon at build time requires a RE-ENCODE of the root (new derived-root bytes), not merely a config toggle. The dormant `_train` plumbing is in place, but the channels themselves are a re-encode. The §4.2 leakage-assert precondition (perturb `exit_cls`/`z_delta` in the harness-held originals) is likewise a v3.1 precondition, to add WITH channel emission (MINOR C-5).

- **S5 (doc) — DG-5 process-dynamics coverage WAIVED/DEFERRED to v3.1.** The bg-job/kill automaton is starved at mint scale: 0 kills / 0 after-launches across the 72 twin seqs and `after`=7 / `kill`=1 across the 360-seq class-freeze pilot (≈1 kill / 360 seqs at mint) — the intervention-floor pacer fills the mutation quota, so `after`/`kill` receive ≈0 draws. DG-5's `{run,stop,term,mixed} ≥15% each` process-state mixture floor is therefore UNSATISFIABLE at mint scale and is WAIVED/DEFERRED to v3.1. The process arm (`ps`/`after`/`kill`) STAYS PRESENT for context but is fitness-excluded/instrument (already excluded from the content pool). This does NOT block the mint: DG-5 is fail-closed and does not bias fitness (the process arm is renormalized out). Do NOT decouple `after` from the mutation-cap and rebuild the automaton pre-mint — that is a re-baseline (NO-GO tripwire). This is the second process-related deferral (jobs/fg/bg/`%n` + literal ctrl-keys already → v4, §5).
...
**2026-07-23 — CLASS FREEZE (freeze-order step 7).** `benchmarks/dockerfs3-classes.json` frozen (`status: FROZEN`); see §4.7 for the full amendment. classes_sha = `08b31deeb16269c2a9d2df338c35d6a6a2f6e733c36d34c7ee5e1c853a2c24e4`. Frozen: axis-2′ 0.96, θ1 0.59, axis-3 0.90, coverage floor 30 pairs + ≥500 steps/content-verb/split; 15 content cells (8 solid + 7 provisional-coverage). `render_canon.py`/`shell_state.py`/`verbsig.py`/`class_measure.py` sha-pinned. MINT-CONFIRMED gates (non-blocking): G-COV grep+composed→§9.3 battery; DG-9 composed 2.05% < 6% (standing breach risk → §4.3 fallback fork if it persists at mint). class_measure re-run reproduces the frozen table bit-for-bit.
